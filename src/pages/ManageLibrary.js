...
// Content Item Modal with Org/Category Selection
const ContentItemModal = ({ isOpen, onClose, onSave, item, organizations, libraryCategories, defaultOrg }) => {
  ...
  // orgCategories: { [orgId]: [categoryId, ...] }
  ...
  useEffect(() => {
    if (isOpen) {
      if (item) {
        ...
        // Default to the selected org if provided
        if (defaultOrg) {
  ...
  const handleSave = async () => {
    ...
    // Validate at least one org with at least one category
  ...
  return (
    <div style={styles.modalOverlay} onClick={onClose}>
      <div ref={modalRef} style={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <h2 style={styles.modalTitle}>{item ? 'Edit Content' : 'Add Content'}</h2>
...
const ManageLibrary = () => {
  ...
  const { libraryCategories, refreshContent } = useContent();
  ...
  // Fetch all content items and their org assignments
  const fetchContent = useCallback(async () => {
    try {
      // Fetch all library content items
  ...
  // Handle saving content with org/category assignments
  ...
  // Handle drag end for moving items between orgs
  const handleDragEnd = async (event) => {
    ...
    // For now, just reorder within the same list
    // Cross-org drag would require more complex logic
  };

  // Prepare items with their assignments for editing
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        <div style={styles.headerInner}>
          ...
          <h1 style={styles.headerTitle}>Manage Library</h1>
          <div style={{ width: 40 }} />
        </div>
      ...
      {deleteConfirm.open && (
        <div style={styles.modalOverlay} onClick={() => setDeleteConfirm({ open: false, item: null })}>
          <div style={styles.deleteModal} onClick={e => e.stopPropagation()}>
            <h2 style={styles.deleteModalTitle}>Delete Content?</h2>
